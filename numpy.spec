...
rm -rf docs-f2py ; mv %{buildroot}%{python_sitearch}/%{pkg_name}/f2py/docs docs-f2py
mv -f %{buildroot}%{python_sitearch}/%{pkg_name}/f2py/f2py.1 f2py.1
...
rm -f %{buildroot}%{python_sitearch}/%{pkg_name}/COMPATIBILITY
rm -f %{buildroot}%{python_sitearch}/%{pkg_name}/DEV_README.txt
rm -f %{buildroot}%{python_sitearch}/%{pkg_name}/INSTALL.txt
rm -f %{buildroot}%{python_sitearch}/%{pkg_name}/LICENSE.txt
rm -f %{buildroot}%{python_sitearch}/%{pkg_name}/README.txt
rm -f %{buildroot}%{python_sitearch}/%{pkg_name}/THANKS.txt
rm -f %{buildroot}%{python_sitearch}/%{pkg_name}/site.cfg.example

%check
cd "%{buildroot}"
%{?scl:scl enable %{scl} - << \EOF}
PYTHONPATH="%{buildroot}%{python_sitearch}" %{__python} -c "import numpy ; numpy.test('full', verbose=3)"
%{?scl:EOF}

%files
%doc docs-f2py LICENSE.txt README.txt THANKS.txt DEV_README.txt COMPATIBILITY site.cfg.example
%dir %{python_sitearch}/%{pkg_name}
%{python_sitearch}/%{pkg_name}/*.py*
%{python_sitearch}/%{pkg_name}/core
%{python_sitearch}/%{pkg_name}/distutils
%{python_sitearch}/%{pkg_name}/doc
%{python_sitearch}/%{pkg_name}/fft
%{python_sitearch}/%{pkg_name}/lib
%{python_sitearch}/%{pkg_name}/linalg
%{python_sitearch}/%{pkg_name}/ma
%{python_sitearch}/%{pkg_name}/numarray
%{python_sitearch}/%{pkg_name}/oldnumeric
%{python_sitearch}/%{pkg_name}/random
%{python_sitearch}/%{pkg_name}/testing
%{python_sitearch}/%{pkg_name}/tests
%{python_sitearch}/%{pkg_name}/compat
%{python_sitearch}/%{pkg_name}/matrixlib
%{python_sitearch}/%{pkg_name}/polynomial
%{python_sitearch}/%{pkg_name}-*.egg-info
%{_includedir}/numpy
